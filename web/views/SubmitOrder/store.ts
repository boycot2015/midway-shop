...
export const useDataStore = defineStore('submitOrderStore', {
  ...
  actions: {
          ...
          if (address.success && address.data) {
            ...
            queryData({
              ...
              goodsList: this.goodsList.map(el => ({
                ...
                num: el.quantity || el.num,
                activityId: el.activityId,
                goodsSkuCode: el.goodsSkuCode,
              })),
            } as OrderParams).then(
              (response: IResponseData<{ goodsList?: Sku[] }>) => {
                if (response.success && response.data) {
                  ...
                  this.payCash =
                    (
                      ((this.totalIntegralPrice - this.totalPayIntegral) * 1) /
                      this.integralRatio
                    ).toFixed(2) || 0;
                } else {
                  ElMessage.error(
                    (response.data as string) || response.message
                  );
                }
                this.loading = false;
              }
    ...
    setPrice() {
      this.payCash =
        (
          ((this.totalIntegralPrice - this.payIntegral) * 1) /
          this.integralRatio
        ).toFixed(2) || 0;
    },
    submitOrder() {
    ...
    getPayData(orderCode: string): Promise<any> {
      return getPayData({ orderCode }).then(
        (response: IResponseData<PayData>) => {
          if (response.success && response.data) {
