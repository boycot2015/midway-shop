...
<template>
    <div class="detail-top">
        ...
        <div class="detail-sku flex">
            <div class="imgs">
                <el-carousel ref="carouselRef" indicator-position="none" :interval="5000" arrow="always" height="500px"
                    v-if="currentSku.goodsSkuImage" @change="val => currentIndicator = val">
                ...
                <div class="indicator flex">
                    <div class="indicator-item" :class="{ 'active': currentIndicator === index }"
                        v-for="(item, index) in currentSku.goodsSkuImage" :key="item.imgName"
                        @click="onChangeCarousel(item, index)">
                ...
                <el-form>
                    ...
                    <div class="box">
                        <el-form-item label="价格">
                            <div>
                                <span class="price-text" v-if="currentSku.isQuotaGoods">￥{{ currentSku.sellingPrice
                                    }}</span>
                                <span class="price-text" v-else>{{ currentSku.integralPrice }} {{
                                    websiteConfig.websiteUnit }}</span>
                            </div>
                        </el-form-item>
                    ...
                    <el-form-item :label="goodsSpec.specName" v-for="(goodsSpec, index) in goodsData.goodsSpecList"
                        :key="goodsSpec.specId">
                        <el-button class="color-item fl" v-if="currentSku.goodsSkuSpec"
                            :type="currentSku.goodsSkuSpec.find(val => goodsSpec.specId === val.specNameCode)?.specValueCode === spec.valueId ? 'primary' : ''"
                            v-for="(spec, index) in goodsSpec.values" :key="spec.valueId"
                            @click="onChangeSku(spec, index)">
                            {{ spec.valueName }}
                        </el-button>
                    </el-form-item>
                    <el-form-item label="数量：">
                        <el-input-number v-model="currentSku.quantity" :step="1" :min="currentSku.minimumOrderQuantity"
